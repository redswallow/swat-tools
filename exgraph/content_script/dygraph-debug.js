...
DygraphLayout.prototype._evaluateLineCharts = function () {
    ...
    for (var f in this.datasets) {
        ...
        for (var b = 0; b < e.length; b++) {
            ...
            var g;
            if (c.logscale) {
                g = 1 - ((Dygraph.log10(parseFloat(d[1])) - Dygraph.log10(c.minyval)) * c.ylogscale)
            } else {
                g = 1 - ((parseFloat(d[1]) - c.minyval) * c.yscale)
            }
            var a = {
    ...
    if (this.options.drawYAxis) {
        if (this.layout.yticks && this.layout.yticks.length > 0) {
            for (var h = 0; h < this.layout.yticks.length; h++) {
                ...
                if (j[0] == 0) {
                    n.style.left = (this.area.x - this.options.yAxisLabelWidth - this.options.axisTickSize) + "px";
    ...
    if (this.options.drawXAxis) {
        if (this.layout.xticks) {
            for (var h = 0; h < this.layout.xticks.length; h++) {
                ...
                n.style.top = (m + this.options.axisTickSize) + "px";
...
DygraphCanvasRenderer.prototype._renderLineChart = function () {
    ...
    var z = this.options.fillAlpha;
    var E = this.layout.options.errorBars;
    var t = this.attr_("fillGraph");
    var e = this.layout.options.stackedGraph;
    var m = this.layout.options.stepPlot;
    var G = [];
    for (var H in this.layout.datasets) {
        if (this.layout.datasets.hasOwnProperty(H)) {
            G.push(H)
        }
    }
    var A = G.length;
    this.colors = {};
    for (var C = 0; C < A; C++) {
        this.colors[G[C]] = p[C % g]
    }
    for (var C = 0; C < this.layout.points.length; C++) {
        var v = this.layout.points[C];
        v.canvasx = this.area.w * v.x + this.area.x;
        v.canvasy = this.area.h * v.y + this.area.y
    }
    var u = d;
    if (E) {
        if (t) {
        ...
        for (var C = 0; C < A; C++) {
            ...
            var h = [-1, -1];
            var D = c.yscale;
            var a = new RGBColor(x);
            var F = "rgba(" + a.r + "," + a.g + "," + a.b + "," + z + ")";
            u.fillStyle = F;
            u.beginPath();
            for (var y = 0; y < this.layout.points.length; y++) {
                var v = this.layout.points[y];
                if (v.name == l) {
                    if (!Dygraph.isOK(v.y)) {
                        k = NaN;
                        continue
                    ...
                    if (m) {
                        var r = [f - v.errorPlus * D, f + v.errorMinus * D];
                        f = v.y
                    } else {
                        var r = [v.y - v.errorPlus * D, v.y + v.errorMinus * D]
                    }
                    ...
                    if (!isNaN(k)) {
                        ...
                        u.lineTo(v.canvasx, r[0]);
                        u.lineTo(v.canvasx, r[1]);
                        if (m) {
                            u.lineTo(k, r[1])
                        } else {
                            u.lineTo(k, h[1])
    ...
    for (var C = 0; C < A; C++) {
        ...
        d.save();
        var v = this.layout.points[0];
        var n = this.dygraph_.attr_("pointSize", l);
        var k = null,
            f = null;
        var w = this.dygraph_.attr_("drawPoints", l);
        var B = this.layout.points;
        for (var y = 0; y < B.length; y++) {
            var v = B[y];
            if (v.name == l) {
                if (!Dygraph.isOK(v.canvasy)) {
                    if (m && k != null) {
                        u.beginPath();
                        u.strokeStyle = x;
                        u.lineWidth = this.options.strokeWidth;
                        u.moveTo(k, f);
                        u.lineTo(v.canvasx, f);
                        u.stroke()
                    }
                    k = f = null
                } else {
                    var o = (!k && (y == B.length - 1 || !Dygraph.isOK(B[y + 1].canvasy)));
                    if (!k) {
                        k = v.canvasx;
                        f = v.canvasy
                    } else {
                    ...
                    if (w || o) {
                        u.beginPath();
                        u.fillStyle = x;
                        u.arc(v.canvasx, v.canvasy, n, 0, 2 * Math.PI, false);
                        u.fill()
                    }
                }
            }
...
Dygraph.intFormat = function (a, b) {
    return a.toString()
};
...
Dygraph.DEFAULT_ATTRS = {
    ...
    yValueFormatter: function (a, b) {
        var d = Dygraph.floatFormat(a, b);
        var c = Dygraph.intFormat(a);
...
Dygraph.startPan = function (n, s, c) {
    c.isPanning = true;
    var j = s.xAxisRange();
    c.dateRange = j[1] - j[0];
...
Dygraph.movePan = function (b, k, c) {
...
Dygraph.prototype.generateLegendHTML_ = function (k, l) {
    if (typeof (k) === "undefined") {
        ...
        for (var e = 1; e < g.length; e++) {
            ...
            var j = this.plotter_.colors[g[e]];
            if (f != "") {
                f += (b ? "<br/>" : " ")
            }
            f += "<b><span style='color: " + j + ";'>&mdash;" + g[e] + "</span></b>"
        }
        return f
    }
    var d = this.numXDigits_ + this.numExtraDigits_;
    var f = this.attr_("xValueFormatter")(k, d) + ":";
    var a = this.attr_("yValueFormatter");
    var m = this.attr_("labelsShowZeroValues");
    var b = this.attr_("labelsSeparateLines");
    for (var e = 0; e < this.selPoints_.length; e++) {
        var n = this.selPoints_[e];
        if (n.yval == 0 && !m) {
            continue
        }
        ...
        var j = this.plotter_.colors[n.name];
        var h = a(n.yval, d);
        f += " <b><span style='color: " + j + ";'>" + n.name + "</span></b>:" + h
    }
    return f
};
...
Dygraph.prototype.addXTicks_ = function () {
    ...
    var d = this.attr_("xTicker");
    var b = d(a[0], a[1], this);
    var c = [];
    if (b.ticks !== undefined) {
        c = b.ticks;
        this.numXDigits_ = b.numDigits
    } else {
        c = b
    }
    ...
    if (a < Dygraph.MONTHLY) {
        ...
        for (var s = q; s <= o; s++) {
            ...
            for (var p = 0; p < e.length; p++) {
                var l = s + "/" + b(1 + e[p]) + "/01";
                var k = Dygraph.dateStrToMillis(l);
                if (k < m || k > h) {
                    continue
                }
                y.push({
...
Dygraph.significantFigures = function (b, f) {
    var c = Math.max(f || 13, 13);
    var e = b.toExponential(c);
    var a = e.lastIndexOf("e");
    for (var d = a - 1; d >= 0; d--) {
        if (e[d] == ".") {
            return 1
        } else {
...
Dygraph.prototype.drawGraph_ = function () {
    ...
    for (var w = D[0].length - 1; w >= 1; w--) {
        ...
        var c = this.attr_("connectSeparatedPoints", w);
        var b = this.attr_("logscale", w);
        var m = [];
        ...
        var r = this.attr_("errorBars") || this.attr_("customBars");
        if (this.dateWindow_) {
        ...
        if (r) {
            for (var v = 0; v < m.length; v++) {
                val = [m[v][0], m[v][1][0], m[v][1][1], m[v][1][2]];
                m[v] = val
            }
        } else {
            if (this.attr_("stackedGraph")) {
...
Dygraph.prototype.computeYAxisRanges_ = function (a) {
    ...
    for (var h in this.seriesToAxisMap_) {
        ...
        var n = this.seriesToAxisMap_[h];
        while (g.length <= n) {
            g.push([])
        }
        g[n].push(h)
    }
    for (var r = 0; r < this.axes_.length; r++) {
        var b = this.axes_[r];
        if (!g[r]) {
            b.extremeRange = [0, 1]
        } else {
        ...
        if (r == 0 || b.independentTicks) {
            var x = Dygraph.numericTicks(b.computedValueRange[0], b.computedValueRange[1], this, b);
            b.ticks = x.ticks;
            this.numYDigits_ = x.numDigits
        } else {
            var k = this.axes_[0];
            var e = k.ticks;
            var f = k.computedValueRange[1] - k.computedValueRange[0];
            var w = b.computedValueRange[1] - b.computedValueRange[0];
            var c = [];
            for (var r = 0; r < e.length; r++) {
                var o = (e[r].v - k.computedValueRange[0]) / f;
                var s = b.computedValueRange[0] + o * w;
                c.push(s)
            }
            var x = Dygraph.numericTicks(b.computedValueRange[0], b.computedValueRange[1], this, b, c);
            b.ticks = x.ticks;
            this.numYDigits_ = x.numDigits
        }
    }
...
Dygraph.dateParser = function (b, a) {
    ...
    if (b.search("-") != -1) {
        ...
        e = Dygraph.dateStrToMillis(c)
    } else {
        if (b.length == 8) {
            c = b.substr(0, 4) + "/" + b.substr(4, 2) + "/" + b.substr(6, 2);
            e = Dygraph.dateStrToMillis(c)
        } else {
            e = Dygraph.dateStrToMillis(b)
        }
    }
    if (!e || isNaN(e)) {
